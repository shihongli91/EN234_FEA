...
subroutine user_element_dynamic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    if ( element_identifier == 10001 .or. element_identifier == 10002) then              ! Basic fully integrated 3D linear elastic element
    ...
    else if ( element_identifier ==700 ) then           ! Stub for a poro_plasticity element

        call el_poro_viscoplasticity_3Ddynamic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
            n_properties, element_properties,element_coords, length_coord_array, &                         ! Input variables
            dof_increment, dof_total, length_dof_array,  &                                                 ! Input variables
            n_state_variables, initial_state_variables, &                                                  ! Input variables
            updated_state_variables,element_residual,element_deleted)                                      ! Output variables
...
subroutine user_element_fieldvariables(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    if ( element_identifier == 1001 .or. element_identifier == 2001 .or. element_identifier == 10001  &
           .or. element_identifier == 10002) then               ! Basic fully integrated 3D linear elastic element
        ...
        call fieldvars_linelast_3dbasic(lmn, element_identifier, n_nodes, node_property_list, &         ! Input variables
            ...
            call fieldvars_hyperslasticity_3dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
                ...
                n_field_variables,field_variable_names, &                                                                ! Field variable definition
                nodal_fieldvariables)      ! Output variables
        else if ( element_identifier == 700) then !  Poro-plasticity
           call fieldvars_3d_porous_plasticity(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
                n_properties, element_properties,element_coords, length_coord_array, &                                   ! Input variables
                dof_increment, dof_total, length_dof_array, &                                                            ! Input variables
                n_state_variables, initial_state_variables,updated_state_variables, &                                    ! Input variables
                n_field_variables,field_variable_names, &                                                                ! Field variable definition
