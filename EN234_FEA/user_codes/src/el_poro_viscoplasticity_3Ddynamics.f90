...
subroutine fieldvars_3d_porous_plasticity(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    real (prec)  ::  Kirch(6),  tr_Kirch,V_f
    real (prec)  :: zero,one,two,three,third,five,Iden(3,3) ! Utilities
    ...
    do kint = 1, n_points
       ...
       tr_Kirch = Kirch(1)+Kirch(2)+Kirch(3)
...
!        write(*,*) stress
        do i=1,3
           Kirch(i) = Kirch(i) - third*tr_Kirch
        end do
        smises = dsqrt( dot_product(Kirch(1:3),Kirch(1:3)) + 2.d0*dot_product(Kirch(4:6),Kirch(4:6)) )*dsqrt(1.5d0)
        jj = jj + 8
...
subroutine nl_poroplas_material(element_properties,n_properties,dstrain,dR,initial_state_variables,updated_state_variables,&
    ...
    real (prec)  ::  dF_mat(2,2),dE_vec(2),sol_new(2),dF_mat_inv(2,2),det_dF_mat,phi_2,relax
  ...
  !**********************************************************************************
   ...
   ! Step 1. Compute the elastic predictors for the deviatoric and hydrostatic stress
     ...
     S_star =  Eyoung/(one+xnu)*dE + matmul(dR,matmul(Tau0_t,transpose(dR)))
   ...
   ! Mises stress
     sig_estar = zero
        ...
        sig_estar = dsqrt(sig_estar)
   ! f_star
     f_star = zero
     fbar_F = (q1+dsqrt(q1**two -q3))/q3
...
!**********************************************************************************************
   ...
   ! Update the volume fraction and matrix strain
       ...
       Vf_tau = one + (Vf_t-one)*exp(-dEv)+ f_N*dEmatrix/s_N/dsqrt(two*pi) &
