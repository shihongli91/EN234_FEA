...
subroutine allocate_mesh_storage
    ...
    if (allocated(zone_namelist)) deallocate(zone_namelist)
    if (allocated(zone_print_flag)) deallocate(zone_print_flag)
    if (allocated(zone_dimension)) deallocate(zone_dimension)
    if (allocated(zone_ndof)) deallocate(zone_ndof)
    if (allocated(displacement_map)) deallocate(displacement_map)
    if (allocated(node_list)) deallocate(node_list)
    if (allocated(coords)) deallocate(coords)
    if (allocated(dof_increment)) deallocate(dof_increment)
    if (allocated(dof_total)) deallocate(dof_total)
    if (allocated(rforce)) deallocate(rforce)
    if (allocated(element_list)) deallocate(element_list)
    if (allocated(element_deleted)) deallocate(element_deleted)
    if (allocated(connectivity)) deallocate(connectivity)
    if (allocated(initial_state_variables)) deallocate(initial_state_variables)
    if (allocated(mesh_subroutine_parameters)) deallocate(mesh_subroutine_parameters)
    if (allocated(updated_state_variables)) deallocate(updated_state_variables)
    if (allocated(element_properties)) deallocate(element_properties)
    if (allocated(densities)) deallocate(densities)
    if (allocated(history_data)) deallocate(history_data)
    if (allocated(subroutine_parameters)) deallocate(subroutine_parameters)
    if (allocated(constraint_parameters)) deallocate(constraint_parameters)
    if (allocated(dof_values)) deallocate(dof_values)
    if (allocated(dload_values)) deallocate(dload_values)
    if (allocated(history_list)) deallocate(history_list)
    if (allocated(history_namelist)) deallocate(history_namelist)
    if (allocated(subroutineparameter_list)) deallocate(subroutineparameter_list)
    if (allocated(subroutineparameter_namelist)) deallocate(subroutineparameter_namelist)
    if (allocated(constraintparameter_list)) deallocate(constraintparameter_list)
    if (allocated(constraintparameter_namelist)) deallocate(constraintparameter_namelist)
    if (allocated(nodeset_list)) deallocate(nodeset_list)
    if (allocated(nodeset_namelist)) deallocate(nodeset_namelist)
    if (allocated(node_lists)) deallocate(node_lists)
    if (allocated(elementset_list)) deallocate(elementset_list)
    if (allocated(elementset_namelist)) deallocate(elementset_namelist)
    if (allocated(element_lists)) deallocate(element_lists)
    if (allocated(prescribeddof_list)) deallocate(prescribeddof_list)
    if (allocated(prescribedforce_list)) deallocate(prescribedforce_list)
    if (allocated(distributedload_list)) deallocate(distributedload_list)
    if (allocated(constraint_list)) deallocate(constraint_list)
    if (allocated(lagrange_multipliers)) deallocate(lagrange_multipliers)
    if (allocated(user_print_parameters)) deallocate(user_print_parameters)
    if (allocated(user_print_filenames)) deallocate(user_print_filenames)
    if (allocated(user_print_units)) deallocate(user_print_units)
    if (allocated(field_variable_names)) deallocate(field_variable_names)
    if (allocated(file_units)) deallocate(file_units)
