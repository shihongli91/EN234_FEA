...
C     Local variables
        ...
        ! Mass density (per unit ref vol)
        double precision Omega              
         ! Angular velocity
        double precision axis(3)            
         ! Rotation axis vector
        double precision x0(3)               
        ! Reference point vector for rotation
        double precision disp(MCRD),pos(MCRD),temp_b1,temp_b2,
     +                   b_vec(MCRD),B_res(mcrd*nnode),
     +                   K_b(1:NDOFEL,1:NDOFEL),
     +                   element_residual(NDOFEL),
     +                   element_stiffness(1:NDOFEL,1:NDOFEL) 
 ...
 ! Obtain current nodal coordinates
      ...
      do i=1,NNODE
         do j=1,MCRD
            COORDC(j,i) = COORDS(j,i) + nodal_displacements(i,j)
         enddo
      enddo
...
C
 ...
 !          Write your code here
       call calculate_shapefunctions(xi(1:MCRD,kint),nnode,MCRD,N,dNdxi)
       dxdxi = matmul(COORDS(1:MCRD,1:nnode),dNdxi(1:nnode,1:MCRD))  
       call invert_small(MCRD,dxdxi,dxidx,determinant)
  
 !    Constructing displacement vector
        disp=0.d0
        pos=0.d0
        disp(1:MCRD) = matmul(nodal_displacements(1:MCRD,1:NNODE),
     +                         N(1:NNODE)) 
     
 !    Constructing coordinates vector
        pos(1:MCRD) = matmul(COORDS(1:MCRD,1:NNODE),N(1:NNODE))

 !    Constructing b vector
      temp_b1 = dot_product((pos(1:MCRD)-x0(1:MCRD)),axis(1:MCRD))
      temp_b2 = dot_product((disp(1:MCRD)-x0(1:MCRD)),axis(1:MCRD))
       b_vec = 0.d0
!       do i=1,MCRD
!        b_vec(i) = b_vec(i) -rho*Omega*Omega*(pos(i)-x0(i)- 
!     +                 temp_b1*axis(i)+ disp(i)-x0(i)-
!     +                 temp_b2*axis(i))
!      enddo
       b_vec(1:MCRD) = -rho*Omega*Omega*(pos(1:MCRD)-x0(1:MCRD)- 
     +                 temp_b1*axis(1:MCRD)+ disp(1:MCRD)-x0(1:MCRD)-
     +                 temp_b2*axis(1:MCRD))
       write(6,*) pos
 !    Constructing B residuals
       B_res = 0.d0  ! B_res(mcrd*nnode)
           
       do i=1, NNODE
          do j=1, MCRD
             B_res((i-1)*MCRD+j)= b_vec(j)*N(i)
          enddo
       enddo
        
        
 !    Constructing Kb matrix. 
       K_b = 0.d0   !
       
      if (MCRD==2) then 
      
      do i=1,nnode
      K_b(1:NDOFEL-1:2,1+2*(i-1))=  N(1:nnode)*N(i)*(1.d0-axis(1)**2.d0)
      K_b(2:NDOFEL:2,1+2*(i-1))  = -N(1:nnode)*N(i)*axis(1)*axis(2)
      K_b(1:NDOFEL-1:2,2+2*(i-1))= -N(1:nnode)*N(i)*axis(1)*axis(2)  
      K_b(2:NDOFEL:2,2+2*(i-1))  =  N(1:nnode)*N(i)*(1.d0-axis(2)**2.d0)
      end do
      
      else if (MCRD==3) then
      
      do i=1,nnode
      K_b(1:NDOFEL-2:3,1+3*(i-1))=  N(1:nnode)*N(i)*(1.d0-axis(1)**2.d0)
      K_b(2:NDOFEL-1:3,1+3*(i-1))= -N(1:nnode)*N(i)*axis(1)*axis(2)
      K_b(3:NDOFEL:3,1+3*(i-1))  = -N(1:nnode)*N(i)*axis(1)*axis(3)
      K_b(1:NDOFEL-2:3,2+3*(i-1))= -N(1:nnode)*N(i)*axis(1)*axis(2)  
      K_b(2:NDOFEL-1:3,2+3*(i-1))=  N(1:nnode)*N(i)*(1.d0-axis(2)**2.d0)
      K_b(3:NDOFEL:3,2+3*(i-1))  = -N(1:nnode)*N(i)*axis(1)*axis(3)
      K_b(1:NDOFEL-2:3,3+3*(i-1))= -N(1:nnode)*N(i)*axis(1)*axis(3) 
      K_b(2:NDOFEL-1:3,3+3*(i-1))= -N(1:nnode)*N(i)*axis(1)*axis(2)
      K_b(3:NDOFEL:3,3+3*(i-1))  =  N(1:nnode)*N(i)*(1.d0-axis(3)**2.d0)
      end do
      
      end if
      
       K_b = -rho*Omega*Omega*K_b
       

! Construting residuls and tangent stiffness
       element_residual(1:NDOFEL)=element_residual(1:NDOFEL)
     +              -  B_res(1:NDOFEL)*w(kint)*determinant
     
       element_stiffness(1:NDOFEL,1:NDOFEL) = 
     +            element_stiffness(1:NDOFEL,1:NDOFEL) +
     +            K_b(1:NDOFEL,1:NDOFEL)*w(kint)*determinant
      
       END DO
!  Return residuals and tangents back to abaqus 
       RHS(1:NDOFEL,1) = element_residual(1:NDOFEL)
       STIF(1:NDOFEL,1:NDOFEL) = element_stiffness(1:NDOFEL,1:NDOFEL)
!      STIF(1:NDOFEL,1:NDOFEL) =  0.d0
       RETURN
       END
!---------------------------------------------------------------------------------
...
!---------------------------------------------------------------------------------       
        ...
        double precision, intent( out ) :: xi(n_coord,*)            
         ! xi(i,a) ith coord of ath integration point
...
!-----------------------------------------------------------------------------------
      ...
      ! Compute the inverse and determinant of a 3x3 or 2x2 matrix
!-----------------------------------------------------------------------------------     
